...
Precedences can be approximated by static constants. The "head" of a task is its earliest start time induced by precedences, while the "tail" is the minimum time required after its completion:$$\mathrm{head}_j = \sum_{i \prec j} p_i$$
